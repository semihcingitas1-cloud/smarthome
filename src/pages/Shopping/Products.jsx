...
import { ShoppingCart, Star, Heart, Filter, ArrowRight, Zap, Package, Truck, Shield, Search, SlidersHorizontal, X, ChevronDown, Grid3x3, List, TrendingUp, Clock, Award, Sparkles, Eye, BarChart3, ArrowUpDown, Percent, Gift, Timer, CreditCard } from "lucide-react";
...
const Products = () => {
  ...
  const benefits = [
    ...
    },
    {
      ...
      description: "2500₺ ve üzeri",
      color: "from-orange-500/10 to-red-500/10",
    },
  ];

  const bageColor = (product) => {
  ...
  const addBasket = () => {
    ...
    dispatch(addToCart({
      ...
      _id: product._id,
      name: product.name,
      price: product.models[0]?.price || product.price,
    ...
    <div className="bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-white min-h-screen transition-colors duration-300">
      ...
      <section className="relative pt-24 pb-16 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold leading-tight mb-6 text-gray-900 dark:text-white">Akıllı Ev<span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 animate-gradient">{" "}Ürünleri</span></h1>
          <p className="text-gray-600 dark:text-slate-400 max-w-2xl mx-auto text-lg mb-8">Evinizi geleceğe taşıyacak en yeni akıllı cihazlar burada. Kalite, güvenlik ve konfor bir arada.</p>

          <div className="max-w-2xl mx-auto relative">

            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-400" size={20} />
            <input type="text" placeholder="Ürün ara..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-800 rounded-2xl pl-12 pr-4 py-4 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-slate-400 focus:outline-none focus:border-blue-500 transition-colors shadow-lg dark:shadow-none"/>
            {searchQuery && ( <button onClick={() => setSearchQuery("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white"><X size={20} /></button> )}
      ...
      <section className="px-6 pb-12">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          {stats.map((stat, i) => ( <div key={i} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 text-center hover:border-blue-500/50 transition-all group shadow-lg dark:shadow-none">
      ...
      <section className="px-6 pb-12">
        ...
        <div className="max-w-7xl mx-auto grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {benefits.map((item, i) => ( <div key={i} className={`bg-gradient-to-br ${item.color} backdrop-blur-sm border border-gray-200 dark:border-slate-800 rounded-2xl p-6 hover:scale-105 transition-all shadow-lg dark:shadow-none`}>
          ...
          <div className="flex items-center gap-3 mb-4 overflow-x-auto pb-2 scrollbar-hide">
            ...
            {categories.map((cat) => {
              ...
              return (
                ...
                <button key={cat.name} onClick={() => setActiveCategory(cat.name)} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all whitespace-nowrap ${activeCategory === cat.name ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg shadow-blue-500/25" : "bg-white dark:bg-slate-900 text-gray-700 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-800 border border-gray-200 dark:border-slate-800"}`}>

                  <Icon size={16} />
                  <span className="font-medium">{cat.name}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${activeCategory === cat.name ? "bg-white/20" : "bg-gray-200 dark:bg-slate-800"}`}>{cat.count}</span>
          ...
          {showFilters && ( <div className="mt-4 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 space-y-6 shadow-lg dark:shadow-none">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <label className="block text-sm font-medium mb-3 text-gray-900 dark:text-white">Minimum Puan</label>

                <div className="flex gap-2">

                  {[0, 3, 4, 5].map((rating) => ( <button key={rating} onClick={() => setSelectedRating(rating)} className={`flex items-center gap-1 px-3 py-2 rounded-lg transition-colors ${selectedRating === rating ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-slate-800 hover:bg-gray-300 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300"}`}>
      ...
      <section className="px-6 pb-24">
        ...
        <div className="max-w-7xl mx-auto mt-5">
          ...
          {filteredProducts.length === 0 ? ( <div className="text-center py-20">
            ...
            <button onClick={() => { setActiveCategory("Tümü"); setSearchQuery(""); setPriceRange([0, 5000]); setSelectedRating(0);}} className="bg-blue-600 px-6 py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors text-white shadow-lg">Filtreleri Sıfırla</button>

          </div> ) : ( <div className={`grid gap-6 ${viewMode === "grid" ? "sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "grid-cols-1 max-w-4xl mx-auto" }`}>

            {filteredProducts.map((product) => ( <div onClick={() => navigate(`/productdetail/${product?.slug}`)} key={product.id} onMouseEnter={() => setHoveredProduct(product.id)} onMouseLeave={() => setHoveredProduct(null)} className={`group bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 hover:border-blue-500/50 rounded-3xl overflow-hidden transition-all hover:shadow-2xl hover:shadow-blue-500/10 ${viewMode === "grid" ? "hover:scale-105" : "flex gap-6"}`}>

              <div className={`relative ${viewMode === "list" ? "w-64 flex-shrink-0" : ""}`}>

                <img src={product.images[0].preview} alt={product.name} className={`object-cover ${viewMode === "grid" ? "h-64 w-full" : "h-full w-full"}`}/>

                <div className="absolute top-4 left-4 flex flex-col gap-2">

                  <div className={`bg-gradient-to-r ${bageColor(product)} px-3 py-1 rounded-full text-xs font-bold shadow-lg text-white`}>{product.badge}</div>
                  {product.discount > 0 && ( <div className="bg-red-500 px-3 py-1 rounded-full text-xs font-bold shadow-lg text-white">%{product.discount}</div>)}

                </div>

                {product.stock < 10 && ( <div className="absolute bottom-4 left-4 bg-orange-500/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 text-white">
                ...
                {hoveredProduct === product.id && viewMode === "grid" && ( <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <div className={`p-6 ${viewMode === "list" ? "flex-1" : ""}`}>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-slate-400">
                    ...
                    <TrendingUp size={12} />
                    {product.salesCount} satıldı

                  </div>

                </div>

                <h3 className="font-bold text-lg mb-3 line-clamp-2 text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{product.name}</h3>

                <div className="flex flex-wrap gap-2 mb-4">

                  {product.features.map((feature, i) => ( <span key={i} className="text-xs bg-gray-200 dark:bg-slate-800 px-2 py-1 rounded-full text-gray-700 dark:text-slate-400">{feature}</span> ))}
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {[...Array(5)].map((_, i) => ( <Star key={i} size={15} className={i < product.ratingsAverage ? "text-yellow-500" : "text-gray-300 dark:text-slate-700"} fill={i < product.ratingsAverage ? "currentColor" : "none"} /> ))}

                  </div>

                  <span className="text-sm text-gray-600 dark:text-slate-400">{product.ratingsAverage} ({product.ratingsQuantity})</span>
                ...
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <span className="line-through text-gray-500 dark:text-slate-500">₺{product.oldPrice.toLocaleString()}</span>
                ...
                <div className="mb-4">
                  ...
                  <div className="h-1.5 bg-gray-200 dark:bg-slate-800 rounded-full overflow-hidden">
                    ...
                    <div className={`h-full rounded-full ${product.stock > 20 ? "bg-green-500" : product.stock > 10 ? "bg-yellow-500" : "bg-red-500"}`}style={{ width: `${Math.min((product.stock / 50) * 100, 100)}%` }} />
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => addBasket()} className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:scale-105 transition-all shadow-lg shadow-blue-500/25 text-white">
                    <ShoppingCart size={18} />
                    <span className="hidden sm:inline">Sepete Ekle</span>
                  </button>

                  <button onClick={() => toggleWishlist(product.id)} className={`bg-gray-200 dark:bg-slate-800 hover:bg-gray-300 dark:hover:bg-slate-700 px-4 rounded-xl text-gray-700 dark:text-white backdrop-blur-sm transition-all ${wishlist.includes(product.id) ? "bg-red-500 scale-110 text-white" : "bg-black/50 hover:bg-red-500 text-white"}`}>
          ...
          {filteredProducts.length > 0 && ( <div className="text-center mt-12">
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-gradient-to-br from-blue-600/20 to-purple-600/20 border border-blue-500/30 rounded-3xl p-8 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-700 dark:text-slate-400 mb-6">Tüm ürünlerde %15 ekstra indirim ve özel avantajlar</p>
          ...
          <div className="bg-gradient-to-br from-orange-600/20 to-red-600/20 border border-orange-500/30 rounded-3xl p-8 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-700 dark:text-slate-400 mb-6">Her alışverişinizde %5 hediye çeki kazanın</p>
      ...
      <section className="px-6 pb-24">
        ...
        <div className="max-w-6xl mx-auto bg-gradient-to-r from-blue-600/20 to-purple-600/20 border border-blue-500/30 rounded-3xl p-12 text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-4xl md:text-5xl font-bold mb-4 text-gray-900 dark:text-white">Premium Paketlerde %30 İndirim</h2>
            <p className="text-gray-700 dark:text-slate-400 mb-8 max-w-2xl mx-auto text-lg">Akıllı evinizi şimdi kurun, avantajlı fiyatlardan yararlanın. Taksit seçenekleri ve ücretsiz kurulum dahil!</p>
      ...
      {wishlist.length > 0 && ( <button onClick={() => navigate('/favorite')} className="fixed bottom-8 right-8 bg-gradient-to-r from-red-500 to-pink-500 w-16 h-16 rounded-full flex items-center justify-center shadow-2xl shadow-red-500/50 hover:scale-110 transition-transform z-40 text-white">

        <Heart fill="currentColor" size={24} />
        <span className="absolute -top-2 -right-2 bg-white text-red-500 w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shadow-lg">{wishlist.length}</span>
